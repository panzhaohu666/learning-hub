...
<h2>二级标题</h2>
<h3>三级标题</h3>
<!-- h1 到 h6，总共六级。通常建议一个页面只放一个 h1（HTML5 允许多个，但不推荐），它是页面最重要的标题 -->
...
/* 如果祖先都没有定位，就相对于初始包含块（通常是根元素 html，大小等于视口，随页面滚动）*/
...
> 📱 记忆口诀：`absolute` 是「找最近的定位祖先当参照系」；没有就退化为相对初始包含块（通常是根元素 html）。这是新手最容易犯的错，加了 absolute 但父元素没写 `position: relative`，元素就飘到页面角落去了。
...
['苹果', '香蕉', '橙子'].includes('苹果');  // true（'苹果' in list）
