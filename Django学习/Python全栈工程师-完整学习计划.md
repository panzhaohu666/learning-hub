...
| TCP/IP 三次握手四次挥手 | [《图解 TCP/IP》](https://book.douban.com/subject/24737674/) 前 5 章 | 3天 | ⭐⭐⭐⭐ |
...
| WebSocket 基础概念 | 了解即可，阶段7深入 | 0.5天 | ⭐⭐ |
...
| Django 中间件、Celery | Python-100-Days Day52,58 | 1.5天 |
...
> **目标**：能用 React + Next.js 开发完整的前端应用
> **检验标准**：独立完成后台管理系统前端（登录 + 列表 + 表单 + 图表）

### 子阶段 5A：Web 前端基础（1.5 周）
...
| 第14周 | **JavaScript 核心**（变量/函数/闭包/异步/Promise） | [现代 JavaScript 教程](https://zh.javascript.info/) | 4天 |

> ⚠️ **JavaScript 基础必须打牢**，不要直接跳到 React。Promise、async/await、数组方法（map/filter/reduce）、解构赋值、模块化（ESM）必须熟练。

### 子阶段 5B：React 核心（2 周）

| 周数 | 主题 | 资源 |
|------|------|------|
| 第15周 | 组件/JSX/Props/State/Hooks | [React 官方文档](https://react.dev/) Quick Start + 教程 |
| 第15周 | **核心 Hooks**：useState/useEffect/useContext/useRef/useMemo/useCallback | 官方文档 + 练习 |
| 第15周 | 状态管理：Zustand（推荐，比 Redux 简单 10 倍） | [Zustand 文档](https://docs.pmnd.rs/zustand) |
| 第16周 | React Router（路由） | [React Router 文档](https://reactrouter.com/) |
| 第16周 | **数据请求**：TanStack Query（React Query） | [TanStack Query 文档](https://tanstack.com/query) |
| 第16周 | 表单处理：React Hook Form + Zod 校验 | 各自官方文档 |

### 子阶段 5C：Next.js + UI 组件库（1.5 周）

| 周数 | 主题 | 资源 |
|------|------|------|
| 第17周 | Next.js App Router / SSR/SSG/ISR | [Next.js 官方教程](https://nextjs.org/learn) |
| 第17周 | API Routes / Server Actions | 同上 |
| 第17周 | TypeScript（基础类型/接口/泛型/工具类型） | [TypeScript 官方 handbook](https://www.typescriptlang.org/docs/handbook/intro.html) |
...
| [React 官方文档](https://react.dev/) | 新版文档极好，直接看英文版 |
| [Next.js 官方教程](https://nextjs.org/learn) | 做 Dashboard 项目就是最好的练习 |
| [TypeScript 官方 handbook](https://www.typescriptlang.org/docs/handbook/intro.html) | 2 小时速通基础，然后边写边查 |
...
```
技术栈：React + Next.js + Ant Design + Zustand + TanStack Query
...
| 第20周 | **Nginx**（反向代理/负载均衡/HTTPS/静态资源） | [Nginx 官方文档](https://nginx.org/en/docs/) + 博客 | 3天 |
| 第21周 | **CI/CD**（GitHub Actions 自动测试/构建/部署） | [GitHub Actions 文档](https://docs.github.com/en/actions) | 2天 |
| 第21周 | **云服务**（阿里云/AWS EC2 基础使用） | 各自控制台文档 + 动手 | 2天 |
...
| 第23周 | **gRPC**（Protocol Buffers + Python 实现） | [gRPC 官方教程](https://grpc.io/docs/languages/python/) | 2天 |
| 第24周 | **API 网关**（Nginx / Kong 概念） | 博客 + 动手 | 1天 |
| 第24周 | **Docker Compose 编排微服务** | 动手实践 | 2天 |
| 第24周 | K8s 入门（Pod/Service/Deployment/Ingress） | [Kubernetes 官方教程](https://kubernetes.io/docs/tutorials/) | 2天 |
...
□ 理解单体 vs 微服务的优缺点
□ 服务拆分原则（按业务领域，不是按技术层）
□ 服务间通信：同步（HTTP/gRPC）vs 异步（消息队列）
□ RabbitMQ：Direct/Topic/Fanout 交换机
□ RabbitMQ：消息确认、持久化、死信队列
□ Celery：任务定义、调度、重试机制
□ gRPC：.proto 定义、代码生成、四种通信模式
□ 分布式追踪基础（了解 Jaeger/Zipkin）
□ Docker Compose 编排多服务
□ K8s 核心概念（Pod/Deployment/Service/ConfigMap/Secret）
```
...
技术栈：FastAPI + Next.js + Ant Design + PostgreSQL + Redis
亮点：
...
技术栈：FastAPI 微服务 + Next.js + PostgreSQL + Redis + RabbitMQ + Docker
亮点：
...
| **3. 网络** | TCP/UDP/HTTP/HTTPS/DNS/CDN/WebSocket/跨域 | 前面阶段2补课内容复习 |
| **4. 操作系统** | 进程线程/死锁/IO 模型/内存管理 | [CS-Notes](http://www.cyc2018.xyz/) |
...
| **7. 系统设计** | 短链接/秒杀/消息推送/IM/排行榜/限流 | [system-design-primer](https://github.com/donnemartin/system-design-primer) |
| **8. 算法** | 数组/链表/栈/队列/树/排序/动态规划 | LeetCode 高频 100 题 |
| **9. 项目复盘** | 项目难点/技术选型/架构演进/遇到的坑 | 整理自己项目的 STAR 描述 |
...
4. **SQL 和 Linux 是隐性加分项**——很多人只重视 Python，后端面试 SQL 问得更多。
5. **不要追新工具**——专注 FastAPI + React + PostgreSQL + Docker + Redis 这五个核心，其他按需补。
6. **面试是水到渠成的事**——项目做好了、基础扎实了，面试准备2周就够了。
