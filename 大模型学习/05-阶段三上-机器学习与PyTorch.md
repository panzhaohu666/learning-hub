...
> 🧮 **数学前置**：如果"导数、矩阵、概率"已经还给了老师，先花 1-2 周过 `../数学学习/机器学习数学速成.md`（线代/微积分/概率/信息论，每个概念都配 NumPy 验证代码）。它按本章的需求反推设计——第 8 周的线性回归、逻辑回归、交叉熵、Softmax 全部提前在数学指南里用 NumPy 写过一遍，你在这里学的是"把 NumPy 换成 PyTorch"。
...
> ⚠️ **常见陷阱**：很多人以为梯度下降保证找到"全局最低点"。不对！如果你的损失函数不是凸的（有很多坑），梯度下降可能卡在"局部最低点"。好在 MSE 是凸函数，所以线性回归不用担心这个问题。后面的神经网络就不是凸的了——那时你会看到 Adam 用动量/自适应学习率加速收敛、减少震荡。
...
> 4 周 × 约 18 小时/周 = 约 72 小时。这是整份计划中最"硬核"的基础部分。你从零开始，一步步走到了能独立搭建 CNN、手写核心组件、用 Wandb 监控训练的水平。
